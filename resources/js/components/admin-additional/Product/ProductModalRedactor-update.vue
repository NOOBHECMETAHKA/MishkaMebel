<template>
    <form @submit.prevent="onSubmit">
        ...
        <div class="form-group">
            ...
            <div class="form-group">
                <button type="submit" class="btn btn-warning mt-3" :class="this.loading ? 'disabled' : ''">Изменить</button>
            </div>
        </div>
    </form>
</template>
...
export default {
    ...
    mounted() {
        ...
        this.fillValue(JSON.parse(this.objProduct));
    },
    methods: {
        fillValue(filledFields){
            this.idProductToUpdate = filledFields.id;
            this.nameProduct = filledFields.name;
            this.descriptionProduct = filledFields.description; //description
            this.priceProduct =  filledFields.price;
            this.guaranteeProduct = this.guaranteesProduct.filter(guarantee => guarantee.name === filledFields.guarantee)[0]; //guarantee
            this.category = this.categories.filter(category => category.category === filledFields.category)[0]; //category.category -> category
            this.selectedObjectFromProductSelect = filledFields.products_product_id; // products_product_id
        ...
        update(){
            this.loading = true;
            axios.post(`/api/products/update/${this.idProductToUpdate}`, {
